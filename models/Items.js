...
module.exports = {
    findAll: function (callback) {
        fs.readFile(path.join(rootPath, 'data/items.json'), 'utf8', function (err, data) {
            if (err) {
                callback(err, null);
            }
            else {
        ...
        fs.readFile(path.join(rootPath, 'data/items.json'), 'utf8', function (err, data) {
            if (err) {
                ...
                try {
                    var data = JSON.parse(data);
                    data.push(item);
                    fs.writeFile(path.join(rootPath, 'data/items.json'), JSON.stringify(data), { encoding: 'utf8' }, function (err) {
                        if (err) {
                            callback(err, null);
                        }
                        else {
